...
def calc_position_data(layer, position_in_layer, results, update_results = True):
    ...
    poyn_TE = (np.real(n * cos_theta * np.conj(Ef_TE + Eb_TE) * (Ef_TE - Eb_TE))) / np.real(n0* cos_theta0)

    Ef_TM = coeff_TM[:,:,:,0] * np.exp(1j * kz * position_in_layer)
    Eb_TM = coeff_TM[:,:,:,1] * np.exp(-1j * kz * position_in_layer)

    poyn_TM = (np.real(n * np.conj(cos_theta) * (Ef_TM + Eb_TM) * np.conj(Ef_TM - Eb_TM))) / (n0 * np.conj(cos_theta0))

    absorb_TE = np.imag(n * cos_theta * kz * np.abs(Ef_TE + Eb_TE)**2) / np.real(n0 * cos_theta0)
    absorb_TM = np.imag(n * np.conj(cos_theta) *
...
def calc_absorption_in_each_layer(thicknesses, results):
    ...
    results.update({
        'absorption_layer_TE': absorption_TE,
        'absorption_layer_TM': absorption_TM
    })
    return results
