...
def test_material_data_conversion_and_interpolation():
    ...
    row = next((row for row in data if jnp.isclose(row[0], expected_frequency_hz, atol=1e-6)), None)
    ...
    assert jnp.isclose(actual_n, expected_n, atol=1e-6), f"Expected n: {expected_n}, but got: {actual_n}"
    assert jnp.isclose(actual_k, expected_k, atol=1e-6), f"Expected k: {expected_k}, but got: {actual_k}"
