...
def test_absorption_1():
    ...
    plt.plot(position, results_coeffs['poyn_TE'].squeeze(axis = 0).T)
    plt.xlabel('position')  # Label for x-axis
    plt.ylabel('poyn_TE')  # Label for y-axis
    plt.title('Plot of position vs. poyn_TE')
    ...
    plt.figure(2)
    plt.clf()
    plt.plot(position, results_coeffs['absorb_TE'].squeeze(axis = 0).T)
    plt.xlabel('position')  # Label for x-axis
    plt.ylabel('absorb_TE')  # Label for y-axis
    plt.legend([f'{wavelength * 1e9:.0f} nm' for wavelength in wavelengths], title='Wavelengths')
    plt.title('Plot of position vs. absor_TE')
